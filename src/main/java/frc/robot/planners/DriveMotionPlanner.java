...
import frc.utilities.physics.DCMotorTransmission;
import frc.utilities.physics.DifferentialDrive;
import frc.utilities.trajectory.DistanceView;
import frc.utilities.trajectory.PurePursuitController;
import frc.utilities.trajectory.Trajectory;
import frc.utilities.trajectory.TrajectoryIterator;
import frc.utilities.trajectory.TrajectorySamplePoint;
import frc.utilities.trajectory.TrajectoryUtil;
import frc.utilities.trajectory.timing.DifferentialDriveDynamicsConstraint;
import frc.utilities.trajectory.timing.TimedState;
import frc.utilities.trajectory.timing.TimingConstraint;
import frc.utilities.trajectory.timing.TimingUtil;
import frc.utilities.util.CSVWritable;
import frc.utilities.util.Units;
...
public class DriveMotionPlanner implements CSVWritable {
    ...
    public void setFollowerType(final FollowerType type) {
        mFollowerType = type;
    }
    ...
    public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(final boolean reversed,
            final List<Pose2d> waypoints, final List<TimingConstraint<Pose2dWithCurvature>> constraints,
            final double max_vel, // inches/s
            final double max_accel, // inches/s^2
    ...
    public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(final boolean reversed,
            final List<Pose2d> waypoints, final List<TimingConstraint<Pose2dWithCurvature>> constraints,
            final double start_vel, final double end_vel, final double max_vel, // inches/s
            final double max_accel, // inches/s^2
            final double max_voltage) {
        ...
        if (reversed) {
            final List<Pose2dWithCurvature> flipped = new ArrayList<>(trajectory.length());
        ...
        final DifferentialDriveDynamicsConstraint<Pose2dWithCurvature> drive_constraints = new DifferentialDriveDynamicsConstraint<>(
                mModel, max_voltage);
        final List<TimingConstraint<Pose2dWithCurvature>> all_constraints = new ArrayList<>();
        all_constraints.add(drive_constraints);
        if (constraints != null) {
            all_constraints.addAll(constraints);
        }
        // Generate the timed trajectory.
        final Trajectory<TimedState<Pose2dWithCurvature>> timed_trajectory = TimingUtil.timeParameterizeTrajectory(
    ...
    public static class Output {
        ...
        public Output(final double left_velocity, final double right_velocity, final double left_accel,
                final double right_accel, final double left_feedforward_voltage,
                final double right_feedforward_voltage) {
        ...
        public void flip() {
            final double tmp_left_velocity = left_velocity;
            left_velocity = -right_velocity;
            right_velocity = -tmp_left_velocity;

            final double tmp_left_accel = left_accel;
            left_accel = -right_accel;
            right_accel = -tmp_left_accel;

            final double tmp_left_feedforward = left_feedforward_voltage;
            left_feedforward_voltage = -right_feedforward_voltage;
            right_feedforward_voltage = -tmp_left_feedforward;
        }
    }

    protected Output updatePID(final DifferentialDrive.DriveDynamics dynamics, final Pose2d current_state) {
        ...
        final double curvature = adjusted_velocity.angular / adjusted_velocity.linear;
    ...
    protected Output updatePurePursuit(final DifferentialDrive.DriveDynamics dynamics, final Pose2d current_state) {
        ...
        //     lookahead_state = new TimedState<>(new Pose2dWithCurvature(lookahead_state.state().getPose()
        //     .transformBy(Pose2d.fromTranslation(new Translation2d((mIsReversed ? -1.0 : 1.0)
        //  * (Constants.kPathMinLookaheadDistance - actual_lookahead_distance), 0.0))), 0.0),
        ...
        final double curvature = 1.0 / Units.inches_to_meters(arc.radius);
        if (Double.isInfinite(curvature)) {
            adjusted_velocity.linear = 0.0;
            adjusted_velocity.angular = dynamics.chassis_velocity.angular;
        } else {
    ...
    protected Output updateNonlinearFeedback(final DifferentialDrive.DriveDynamics dynamics,
        ...
        final DifferentialDrive.WheelState feedforward_voltages = mModel.solveInverseDynamics(dynamics.chassis_velocity,
    ...
    public Output update(final double timestamp, final Pose2d current_state) {
        ...
        mLastTime = timestamp;
        final TrajectorySamplePoint<TimedState<Pose2dWithCurvature>> sample_point = mCurrentTrajectory.advance(mDt);
