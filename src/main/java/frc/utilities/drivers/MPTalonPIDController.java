...
public class MPTalonPIDController {
	...
	public MPTalonPIDController(long periodMs, ArrayList<TalonFXEncoder> motorControllers) 
	{
		...
		if (controlMode == MPControlMode.STRAIGHT) {
			...
			for (TalonFXEncoder motorController : motorControllers) {
				if (motorController.isRight()) {
					motorController.config_kF(0, KfRight, TalonFXEncoder.TIMEOUT_MS);
					motorController.setWorld(ControlMode.Position, mpPoint.position);
				}
				else {
					motorController.config_kF(0, KfLeft, TalonFXEncoder.TIMEOUT_MS);
					motorController.setWorld(ControlMode.Position, mpPoint.position);
				}
			}
		}
	...
	public void setTarget(double position, double Kf) {
		...
		for (TalonFXEncoder motorController : motorControllers) {
			motorController.config_kF(0, KfPerPosition, TalonFXEncoder.TIMEOUT_MS);
