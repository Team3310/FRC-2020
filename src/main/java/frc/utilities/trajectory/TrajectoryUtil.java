...
import frc.utilities.geometry.IPose2d;
import frc.utilities.geometry.Pose2d;
import frc.utilities.geometry.Pose2dWithCurvature;
import frc.utilities.geometry.State;
import frc.utilities.geometry.Twist2d;
import frc.utilities.spline.QuinticHermiteSpline;
import frc.utilities.spline.SplineGenerator;
...
public class TrajectoryUtil {
    public static <S extends IPose2d<S>> Trajectory<S> mirror(final Trajectory<S> trajectory) {
        final List<S> waypoints = new ArrayList<>(trajectory.length());
    ...
    public static <S extends IPose2d<S>> Trajectory<TimedState<S>> mirrorTimed(
        ...
        final List<TimedState<S>> waypoints = new ArrayList<>(trajectory.length());
        for (int i = 0; i < trajectory.length(); ++i) {
            final TimedState<S> timed_state = trajectory.getState(i);
    ...
    public static <S extends IPose2d<S>> Trajectory<S> transform(final Trajectory<S> trajectory,
            final Pose2d transform) {
        final List<S> waypoints = new ArrayList<>(trajectory.length());
        for (int i = 0; i < trajectory.length(); ++i) {
    ...
    public static <S extends State<S>> Trajectory<S> resample(final TrajectoryView<S> trajectory_view,
        ...
        final ArrayList<S> states = new ArrayList<S>(num_states);
    ...
    public static Trajectory<Pose2dWithCurvature> trajectoryFromPathFollower(final IPathFollower path_follower,
            final Pose2dWithCurvature start_state, final double step_size, final double dcurvature_limit) {
        final List<Pose2dWithCurvature> samples = new ArrayList<Pose2dWithCurvature>();
        ...
        while (!path_follower.isDone()) {
            ...
            final Twist2d average_steering_command = !curvature_valid ? steering_command
    ...
    //         final double maxDx, final double maxDy, final double maxDTheta) {
    //     final List<QuinticHermiteSpline> splines = new ArrayList<>(waypoints.size() - 1);
